import React from 'react'
import './components.css'
import { useNavigate } from 'react-router-dom'

function Coursees() {

    const navigate = useNavigate()

    return (
        <div className='course'>
            <div className="course-container">
                <div className="course-parent">
                    <div onClick={() => navigate('/frontend')} className="course-card">
                        <h3>Frontend dasturlash kurslari</h3>
                        <ul>
                            <li>- Haftasiga 3 kun</li>
                            <li>- Kuniga 2 soatdan</li>
                            <li>- Davomiyligi 10 oygacha</li>
                            <li>- Narx oyiga 1 200 000 so'm</li>
                            <li>- Ikkiga bo'lib to'lash mumkin</li>
                            <li>
                                <div className="images">
                                    <img src="https://upload.wikimedia.org/wikipedia/commons/thumb/6/61/HTML5_logo_and_wordmark.svg/800px-HTML5_logo_and_wordmark.svg.png" alt="" />
                                    <img src="https://upload.wikimedia.org/wikipedia/commons/thumb/d/d5/CSS3_logo_and_wordmark.svg/640px-CSS3_logo_and_wordmark.svg.png" alt="" />
                                    <img src="https://upload.wikimedia.org/wikipedia/commons/thumb/9/99/Unofficial_JavaScript_logo_2.svg/800px-Unofficial_JavaScript_logo_2.svg.png" alt="" />
                                    <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQcR5U16C8yXgBpl7-Bc7Itjx3_LRl425zINA&s" alt="" />
                                    <img src="https://upload.wikimedia.org/wikipedia/commons/thumb/c/c2/GitHub_Invertocat_Logo.svg/220px-GitHub_Invertocat_Logo.svg.png" alt="" />
                                </div>
                            </li>
                        </ul>
                        <img className='course-icon' src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQcR5U16C8yXgBpl7-Bc7Itjx3_LRl425zINA&s" alt="" />

                    </div>
                    <div onClick={() => navigate('/backend')} className="course-card">
                        <h3>Python tilida Backend kurslari </h3>
                        <ul>
                            <li>- Haftasiga 3 kun</li>
                            <li>- Kuniga 2 soatdan</li>
                            <li>- Davomiyligi 10 oygacha</li>
                            <li>- Narx oyiga 1 200 000 so'm</li>
                            <li>- Ikkiga bo'lib to'lash mumkin</li>
                            <li>
                                <div className="images">
                                <img src="https://upload.wikimedia.org/wikipedia/commons/thumb/c/c3/Python-logo-notext.svg/1200px-Python-logo-notext.svg.png" alt="" />
                                        <img src="https://static-00.iconduck.com/assets.00/django-icon-402x512-tkdfpj8s.png" alt="" />
                                    <img src="https://uxwing.com/wp-content/themes/uxwing/download/web-app-development/api-icon.png" alt="" />
                                    <img src="https://flowxo.com/wp-content/uploads/2021/03/Telegram-Logo.png" alt="" />
                                    <img src="https://upload.wikimedia.org/wikipedia/commons/thumb/c/c2/GitHub_Invertocat_Logo.svg/220px-GitHub_Invertocat_Logo.svg.png" alt="" />

                                </div>
                            </li>
                        </ul>
                        <img className='course-icon' src="https://upload.wikimedia.org/wikipedia/commons/thumb/c/c3/Python-logo-notext.svg/1200px-Python-logo-notext.svg.png" alt="" />

                    </div>
                    <div onClick={() => navigate('/mobile')} className="course-card">
                        <h3>Golang tilida Backend kurslari </h3>
                        <ul>
                            <li>- Haftasiga 3 kun</li>
                            <li>- Kuniga 2 soatdan</li>
                            <li>- Davomiyligi 10 oygacha</li>
                            <li>- Narx oyiga 1 200 000 so'm</li>
                            <li>- Ikkiga bo'lib to'lash mumkin</li>
                            <li>
                                <div className="images">
                                <img src="https://go.dev/blog/go-brand/Go-Logo/PNG/Go-Logo_Blue.png" alt="" />
                                    <img src="https://static-00.iconduck.com/assets.00/django-icon-402x512-tkdfpj8s.png" alt="" />
                                    <img src="https://uxwing.com/wp-content/themes/uxwing/download/web-app-development/api-icon.png" alt="" />
                                    <img src="https://flowxo.com/wp-content/uploads/2021/03/Telegram-Logo.png" alt="" />
                                    <img src="https://upload.wikimedia.org/wikipedia/commons/thumb/c/c2/GitHub_Invertocat_Logo.svg/220px-GitHub_Invertocat_Logo.svg.png" alt="" />

                                </div>
                            </li>
                        </ul>
                        <img className='course-icon' src="https://go.dev/blog/go-brand/Go-Logo/PNG/Go-Logo_Blue.png" alt="" />

                    </div>
                    <div className="course-card" onClick={() => navigate('/frontend')}>
                        <h3>QA Engineer dasturlash kurslari</h3>
                        <ul>
                            <li>- Haftasiga 3 kun</li>
                            <li>- Kuniga 2 soatdan</li>
                            <li>- Davomiyligi 10 oygacha</li>
                            <li>- Narx oyiga 1 200 000 so'm</li>
                            <li>- Ikkiga bo'lib to'lash mumkin</li>
                            <li>
                                <div className="images">
                                    <img src="https://static-00.iconduck.com/assets.00/firefox-icon-1981x2048-4izjijq3.png" alt="" />
                                    <img src="https://cdn4.iconfinder.com/data/icons/inspection/512/inspection-qa-qualify-02-512.png" alt="" />
                                    <img src="https://cdn1.iconfinder.com/data/icons/android-png/256/Android-Opera-Mini.png" alt="" />
                                </div>
                            </li>
                        </ul>
                        <img className='course-icon' src="https://cdn4.iconfinder.com/data/icons/inspection/512/inspection-qa-qualify-02-512.png" alt="" />

                    </div>
                    <div className="course-card" onClick={() => navigate('/frontend')}>
                        <h3>Ilova yaratadigan Mobile Flutter kurslari </h3>
                        <ul>
                            <li>- Haftasiga 3 kun</li>
                            <li>- Kuniga 2 soatdan</li>
                            <li>- Davomiyligi 10 oygacha</li>
                            <li>- Narx oyiga 1 200 000 so'm</li>
                            <li>- Ikkiga bo'lib to'lash mumkin</li>
                            <li>
                                <div className="images">
                                <img src="https://storage.googleapis.com/cms-storage-bucket/a9d6ce81aee44ae017ee.png" alt="" />
                                    <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTxRk9iJdQgPS35Xcc0OXIKvqfmBVaW4f2s1g&s" alt="" />
                                    <img src="https://upload.wikimedia.org/wikipedia/commons/thumb/7/74/Kotlin_Icon.png/1200px-Kotlin_Icon.png" alt="" />
                                    <img src="https://upload.wikimedia.org/wikipedia/commons/thumb/c/c2/GitHub_Invertocat_Logo.svg/220px-GitHub_Invertocat_Logo.svg.png" alt="" />
                                </div>
                            </li>
                        </ul>
                        <img className='course-icon' src="https://storage.googleapis.com/cms-storage-bucket/a9d6ce81aee44ae017ee.png" alt="" />

                    </div>
                    <div className="course-card" onClick={() => navigate('/frontend')}>
                        <h3>Kids kurslari</h3>
                        <ul>
                            <li>- Haftasiga 3 kun</li>
                            <li>- Kuniga 2 soatdan</li>
                            <li>- Davomiyligi 10 oygacha</li>
                            <li>- Narx oyiga 1 200 000 so'm</li>
                            <li>- Ikkiga bo'lib to'lash mumkin</li>
                            <li>
                                <div className="images">
                                    <img src="https://static-00.iconduck.com/assets.00/openai-icon-1011x1024-uztb7qme.png" alt="" />
                                    <img src="https://cdn-icons-png.flaticon.com/512/919/919846.png" alt="" />
                                    <img src="https://iconape.com/wp-content/png_logo_vector/microsoft-office-2013.png" alt="" />
                                </div>
                            </li>
                        </ul>
                        <img className='course-icon' src="https://cdn-icons-png.flaticon.com/512/919/919846.png" alt="" />

                    </div>
                </div>
            </div>
        </div>
    )
}

export default Coursees